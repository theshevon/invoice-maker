...
class Mailer:
    ...
    def __init__(self, use_prod):
        ...
        with open(PATH_TO_EMAIL_TEMPLATE) as f:
            self.EMAIL_TEMPLATE = f.read().format(PATH_TO_ICON_IMG)

        try:  
            if use_prod:
                self.logger.info("Attempting to connect to production email server...")
                self.smtp = smtplib.SMTP_SSL("smtp.gmail.com", 465)
                self.username = os.environ.get("EMAIL_USER")
                self.smtp.login(self.username, os.environ.get("EMAIL_PASS"))
            else:
                self.logger.info("Attempting to connect to test email server")
    ...
    def send_email(self, student_name, student_email, path_to_pdf):
        ...
        try:
            ...
            self.logger.info(f"Sent invoice to { student_name } <{ student_email }>")

            return True

        except:

            self.logger.error(f"Error in trying to send email to { student_name } <{ student_email }>!", exc_info=True)
            return False
